...
function Indexpage() {
  ...
  useEffect(() => {
    const fetchPlaces = async () => {
      try {
        const response = await axios.get(`https://homelytics-app.onrender.com/all/places`);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-9">
      {places.map((place, index) => {
        ...
        return (
          ...
          >
            <img
              src={`https://homelytics-app.onrender.com/${slicedUrl}`}
              alt={place.title}
              className="w-full object-cover rounded-t-lg border border-purple-000"
              style={{ maxHeight: "250px" }} // Limiting the max height for consistency
            />
