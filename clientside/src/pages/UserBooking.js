    ...
    const fetchBookings = async () => {
      try {
        ...
        const response = await fetch(
          `https://homelytics-app.onrender.com/user/bookings?userId=${userId}`
        );
        if (response.ok) {
    ...
    const fetchPlaceDetails = async () => {
      try {
        const promises = bookings.map(async (booking) => {
          const response = await fetch(
            `https://homelytics-app.onrender.com/place/${booking.placeId}`
          );
          if (response.ok) {
            const data = await response.json();
      ...
      {loading ? (
        ...
        <div className="flex flex-wrap justify-center">
          ...
          {bookings.map((booking) => (
            <div
              key={booking._id}
              className="w-full sm:w-40 md:w-48 lg:w-96 p-2"
            >
                ...
                {placeDetails[booking.placeId] && (
                  <div>
                    ...
                    {placeDetails[booking.placeId].photos && (
                      <img
                        src={`https://homelytics-app.onrender.com/${placeDetails[
                          booking.placeId
                        ].photos.slice(7)}`}
                        alt="Place"
