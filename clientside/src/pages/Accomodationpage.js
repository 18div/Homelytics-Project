...
function Accomodationpage() {
      ...
      try {
        if (userIdFromCookie) {
          const response = await axios.get(
            `https://homelytics-project.onrender.com/places/${userIdFromCookie}`
          );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://homelytics-project.onrender.com/places/delete/${id}`);
      setAccommodations(
          ...
          {accommodations.map((accommodation, index) => {
            ...
            return (
              ...
              >
                <div className="mr-4">
                  <img
                    src={`https://homelytics-project.onrender.com/${slicedUrl}`}
