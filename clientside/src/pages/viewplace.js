...
function PlacePage() {
  ...
  useEffect(() => {
    const fetchPlace = async () => {
      try {
        const response = await axios.get(`https://homelytics-app.onrender.com/place/${id}`);
  ...
  const handleBook = async () => {
    ...
    try {
      const userBookings = await axios.get(`https://homelytics-app.onrender.com/user/bookings?userId=${userId}`);
      ...
      if (overlappingBooking) {
        ...
        const response = await axios.post("https://homelytics-app.onrender.com/book", bookingData);
    ...
    <div className="container mx-auto px-2 py-8 relative">
      {url && (
        ...
        <div className="relative mb-8">
          <img
            src={`https://homelytics-app.onrender.com/${url}`}
            alt={place.title}
      ...
      <div className={`flex ${isSmallScreen ? "flex-col" : "flex-row"} items-start`}>
        ...
        {isSmallScreen ? (
          <div className="flex flex-col mt-4">
            {url2 && (
              <img
                src={`https://homelytics-app.onrender.com/${url2}`}
                alt="Room"
            ...
            {url3 && (
              <img
                src={`https://homelytics-app.onrender.com/${url3}`}
                alt="Lobby"
